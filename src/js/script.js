...
myQuestions.each( function(index){
    ...
    if (index == 0){
        $(this).find('.previous').remove(); //remove previous button from the first question
    }
...
$('.previous, .next, .missed, .take-another-quiz').click (function (){
    if ($(this).hasClass('previous')){
        currQ-=2; //because showQ always increments by 1, we need to set the currQ by 2 and THEN showQ
